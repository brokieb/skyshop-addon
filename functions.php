...
function restGetFile($event,$fileName,$token){
    $ch = curl_init($event);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    curl_setopt($ch, CURLOPT_HTTPHEADER, [
        "Authorization:bearer ".$token
...
function GetBaselinker($method,$parameter)
    {
        $url = 'https://api.baselinker.com/connector.php';

        $curlPost = 'token=3458-22173-GIS9FM9VCJ5IAKTUIX9S0E7YG7JL34D391DLROK60DSUMPCEE1SX6CU5INI9JT4B&method=' . $method. '&parameters=' . json_encode($parameter);
        $ch = curl_init();
