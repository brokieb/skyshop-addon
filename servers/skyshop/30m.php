...
echo $who = "SELECT * FROM `user` INNER JOIN `bundle` ON `user`.`bundle_id`=`bundle`.`bundle_id` INNER JOIN `connected_shop` ON `connected_shop`.`user_id`=`user`.`user_id` WHERE `bundle_script_time`='30m'";
...
if($a>$stan[$z->prod_id]){
    echo 
    $sql2 = "UPDATE `zestaw` SET `ss_ilosc`='".$a."' WHERE `id_produktu`=".$z->prod_id." ";
